	...
	.p-catalog.container(:class="!onMap?'p-catalog--map':''")
		...
		.p-catalog__center
			.aside(:class="!onMap?'aside--map':''")
				work-cirle(v-if="!onMap" @change="e=>workCircleData=e" v-model="isWorkCircle")
				.aside__prop
					label.on-map
			...
			.p-catalog__items(v-if="onMap")
				...
				pag(v-if="totalCountItems > 0" :total-count="totalCountItems" :size="parseInt(sizePerPage)" v-model="currentPage")
